...
void NumericalIntegrator::setBoundsFromText(const std::string &lowerText,
                                            const std::string &upperText) {
  EquationParser temp;
...
void NumericalIntegrator::setPoints(int numPoints) {
  if (numPoints < 2)
    throw std::runtime_error("Number of points must be >= 2");
  n = numPoints;
}
...
void NumericalIntegrator::loadTable(
...
/**
 * @brief Computes integral using the Trapezoidal Rule.
 * Formula: (h/2) * [ f(x0) + 2*sum(f(xi)) + f(xn) ]
 */
...
/**
 * @brief Computes integral using Simpson's 1/3 Rule.
 * Formula: (h/3) * [ f(x0) + 4*sum(odd) + 2*sum(even) + f(xn) ]
 * Constraint: Number of intervals must be even (n is odd).
 */
...
/**
 * @brief Computes integral using Simpson's 3/8 Rule.
 * Formula: (3h/8) * [ f(x0) + 3*sum(others) + 2*sum(multiples of 3) + f(xn) ]
 * Constraint: Number of intervals must be a multiple of 3.
 */
double NumericalIntegrator::simpsons38Rule() {
  if ((n - 1) % 3 != 0)
    throw std::runtime_error(
        "Simpson's 3/8 Rule requires intervals divisible by 3.");
